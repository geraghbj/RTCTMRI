function varargout = ct2nii(src, niiFolder, fmt)
...
for i = 1:nRun, disp(num2str(i));a = strfind(rNames{i}, '_s'); j_s(i) = a(end)-1; end
for i = 1:nRun
...
% BJG fixes duplicate field problem
[~,idxu,idxc] = unique(fnames);
[count, ~, idxcount] = histcounts(idxc,numel(idxu));
idxkeep = count(idxcount)>1;
dup_ind = find(idxkeep,1,'last');
if ~isempty(dup_ind)
    fnames{dup_ind} = [fnames{dup_ind} '_duplicate'];
end
...
% alau@sri: save out all headers, instead of just 1
h_all = cell2struct(h_all, fnames, 2); % convert into struct
fname = [niiFolder 'dcmHeadersAll.mat'];
if exist(fname, 'file') % if file exists, we update fields only
    S = load(fname);
    for i = 1:numel(fnames), S.h.(fnames{i}) = h_all.(fnames{i}); end
    h_all = S.h; %#ok
end
save(fname, 'h_all', '-v7'); % -v7 better compatibility
...
%% Subfunction: return NumberOfImagesInMosaic in Siemens mosaic, or [] otherwise.
